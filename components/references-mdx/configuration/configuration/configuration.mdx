...
<P>
  The following example will pass the <InlineCode>MY_KEY</InlineCode> static env
  to all <Link href="/docs/v2/build-step">Builds</Link> and
...
<Note type="alert">
  ...
  <br />
  <br />
  If you'd like to customize your Serverless Functions, use the <Link href="/docs/configuration#project/functions">
    functions
  </Link> property instead. If you'd like to deploy a monorepo, learn more about
  how to do it <Link href="/docs/v2/git-integrations#monorepos">here</Link>.
</Note>
...
<Note type="warning">
  When using Next.js, only <InlineCode>memory</InlineCode> and <InlineCode>maxDuration</InlineCode> can be used.
</Note>
...
<Note type="warning">
  Deploying Serverless Functions to multiple regions is limited to{' '}
  <Link href="/pricing">Enterprise plans</Link> and deploying to a specified
  region is limited to <Link href="/pricing">Pro plans</Link>. Read more about
  these limits on{' '}
  <Link href="/docs/v2/platform/limits#serverless-function-regions">
    the Limits page
  </Link>
  .
</Note>
...
<Code lang="json">{`{
  "redirects": [
    ...
    { "source": "/wp-content/image/(.*)", "destination": "https://storage.googleapis.com/$1" },
